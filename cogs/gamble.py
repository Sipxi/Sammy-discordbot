...
class Gamble(CustomCog):
    ...
    @commands.command(help="Gives a random number between 1 and 100")
    async def roll(self, ctx)-> None:
        n = random.randrange(1, 101)
        await ctx.send(n)

    @commands.command(help="Random number between 1 and 6")
    async def dice(self, ctx)-> None:
        n = random.randrange(1, 6)
        await ctx.send(n)

    @commands.command(help="Either Heads or Tails")
